...
exports.AddAgencyByAdmin = catchAsyncErrors(async (req, res,next) => {
    ...
    const message =`Votre compte a été accepté par l'administrateur 
    ...
    try {
        await sendEmail({
            email: agency.email,
            subject: 'MalaBus compte accepter',
            message
        })
        res.status(200).json({
            success: true,
...
exports.RefusedAgencyByAdmin = catchAsyncErrors(async (req, res,next) => {
    ...
    if (agency.verified==false)
    {
        try {
            await sendEmail({
                email: agency.email,
                subject: 'MalaBus compte refuser',
                message
            })
            res.status(200).json({
                success: true,
                message: `Email sent to: ${agency.email}`,
                messageError :`Votre compte a été supprimer par l'administrateur `
            }
            )
        }
        catch (error) {
